...
//	List<Vector3f> normals = new ArrayList<Vector3f>();
	...
	public MeshData extractModelData(Vector3f scale){
		readRawData(scale);
		assembleVertices();
	...
	private void readRawData(Vector3f scale) {
		readPositions(scale);
		readNormals();
		readTextureCoords();
	}

	private void readPositions(Vector3f scale) {
		...
		for (int i = 0; i < count/3; i++) {
			float x = scale.x * Float.parseFloat(posData[i * 3]);
			float y = scale.y * Float.parseFloat(posData[i * 3 + 1]);
			float z = scale.z * Float.parseFloat(posData[i * 3 + 2]);
